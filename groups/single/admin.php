...
<?php if ( bp_is_group_admin_screen( 'edit-details' ) ) : ?>
	...
	<input type="text" name="group-name" id="group-name" class="input-xxlarge" value="<?php bp_group_name(); ?>" aria-required="true" />

	<label for="group-desc"><?php _e( 'Group Description (required)', 'buddypress' ); ?></label>
	<textarea name="group-desc" id="group-desc" aria-required="true" class="input-xxlarge" ><?php bp_group_description_editable(); ?></textarea>
	...
	<p><input type="submit" class="btn btn-success btn-small" value="<?php _e( 'Save Changes', 'buddypress' ); ?>" id="save" name="save" /></p>
...
<?php if ( bp_is_group_admin_screen( 'group-avatar' ) ) : ?>
	...
	<?php if ( 'upload-image' == bp_get_avatar_admin_step() ) : ?>
			...
			<p>
				...
				<input type="submit" name="upload" id="upload" class="btn btn-success" value="<?php _e( 'Upload Image', 'buddypress' ); ?>" />
			...
			<?php if ( bp_get_group_has_avatar() ) : ?>
				...
				<?php bp_button( array( 'class' => 'btn btn-danger', 'id' => 'delete_group_avatar', 'component' => 'groups', 'wrapper_id' => 'delete-group-avatar-button', 'link_class' => 'edit', 'link_href' => bp_get_group_avatar_delete_link(), 'link_title' => __( 'Delete Avatar', 'buddypress' ), 'link_text' => __( 'Delete Avatar', 'buddypress' ) ) ); ?>
...
<?php if ( bp_is_group_admin_screen( 'manage-members' ) ) : ?>
	...
	<div class="page-header bp-widget">
		<h4><?php _e( 'Administrators', 'buddypress' ); ?></h4>

		<?php if ( bp_has_members( '&include='. bp_group_admin_ids() ) ) : ?>
		
		<ul id="admins-list" class="unstyled item-list single-line">
			
			<?php while ( bp_members() ) : bp_the_member(); ?>
			<li>
				<div class="pull-left">
					<?php echo bp_core_fetch_avatar( array( 'class' => 'img-polaroid', 'item_id' => bp_get_member_user_id(), 'type' => 'thumb', 'width' => 30, 'height' => 30, 'alt' => sprintf( __( 'Profile picture of %s', 'buddypress' ), bp_get_member_name() ) ) ); ?>
				</div>
				<div class="pull-left item-wrapper">
					<div><a href="<?php bp_member_permalink(); ?>"> <?php bp_member_name(); ?></a></div>
					<h5>
...
<?php if ( bp_is_group_admin_screen( 'delete-group' ) ) : ?>
	...
	<div class="submit">
		<input type="submit" class="btn btn-danger" disabled="disabled" value="<?php _e( 'Delete Group', 'buddypress' ); ?>" id="delete-group-button" name="delete-group-button" />
	</div>
